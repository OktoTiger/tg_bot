...
def start_message_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        text="Привет!\nБот генерирует тестовых пользователей. "\
...
def message_handler(message: types.Message):
    ...
    if message.text == "1️⃣":
        ...
        bot.send_message(chat_id=message.chat.id, text="Не понимаю тебя.Попробуй выбрать количество необходмых пользователей")
        return
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text="Для получения новых данных, можешь выбрать еще раз 👇🏻",
        reply_markup=main_menu_reply_markup
    )
